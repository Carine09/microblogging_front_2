...
<div
        ...
        <div class="flex item-baseline">
          ...
          />
          <h2 class="font-heading text-dark-green text-xl my-2">{{ props.name }}</h2> <!-- à verifier -->
        </div>
        <div class="h-screen">
          ...
          />
          <p class="text-body text-dark-green my-1 md:my-2">{{ technic_id.name }}</p>
        </div>
      </div>
</template>
...
const props = defineProps({
    ...
    name: String,
    technic_id: [Object, String, Number],
    text: String,
    img_url: String,
    })
</script>